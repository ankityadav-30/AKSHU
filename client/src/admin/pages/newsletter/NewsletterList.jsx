// APP/client/src/admin/pages/newsletter/NewsletterList.jsx

import { useEffect, useState } from "react";
import { HiNewspaper, HiRefresh } from "react-icons/hi";
import api from "../../../services/api.js";

const NewsletterList = () => {
    const [subscribers, setSubscribers] = useState([]);
    const [loading, setLoading] = useState(true);

    const fetchSubscribers = () => {
        setLoading(true);
        api.get("/newsletter/subscribed")
            .then((res) => setSubscribers(res.data?.data?.subscribers || res.data?.data || []))
            .catch(() => {})
            .finally(() => setLoading(false));
    };

    useEffect(() => {
        fetchSubscribers();
    }, []);

    return (
        <div className="admin-page">
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "var(--space-6)", flexWrap: "wrap", gap: "var(--space-4)" }}>
                <div>
                    <h1 style={{ fontSize: "var(--font-size-2xl)", color: "var(--color-text-primary)", marginBottom: "var(--space-1)" }}>
                        Newsletter Subscribers
                    </h1>
                    <p style={{ fontSize: "var(--font-size-xs)", color: "var(--color-text-secondary)" }}>
                        Manage AKSHU Insights newsletter subscribers.
                    </p>
                </div>
                <button type="button" className="btn btn--outline" onClick={fetchSubscribers}>
                    <HiRefresh /> Refresh
                </button>
            </div>

            <div style={{ background: "var(--color-bg-surface)", border: "1px solid var(--color-border)", borderRadius: "var(--radius-xl)", overflow: "hidden" }}>
                <div style={{ overflowX: "auto" }}>
                    <table style={{ width: "100%", borderCollapse: "collapse", textAlign: "left", fontSize: "var(--font-size-sm)" }}>
                        <thead>
                            <tr style={{ background: "rgba(255,255,255,0.02)", borderBottom: "1px solid var(--color-border)" }}>
                                <th style={{ padding: "var(--space-4)", color: "var(--color-text-tertiary)", fontWeight: "bold" }}>Subscriber Email</th>
                                <th style={{ padding: "var(--space-4)", color: "var(--color-text-tertiary)", fontWeight: "bold" }}>Status</th>
                                <th style={{ padding: "var(--space-4)", color: "var(--color-text-tertiary)", fontWeight: "bold" }}>Subscribed Date</th>
                            </tr>
                        </thead>
                        <tbody>
                            {loading ? (
                                <tr>
                                    <td colSpan={3} style={{ padding: "var(--space-8)", textAlign: "center", color: "var(--color-text-secondary)" }}>
                                        Loading subscribers...
                                    </td>
                                </tr>
                            ) : subscribers.length === 0 ? (
                                <tr>
                                    <td colSpan={3} style={{ padding: "var(--space-8)", textAlign: "center", color: "var(--color-text-secondary)" }}>
                                        <HiNewspaper size={32} style={{ opacity: 0.3, marginBottom: "var(--space-2)" }} />
                                        <p>No subscribers yet.</p>
                                    </td>
                                </tr>
                            ) : (
                                subscribers.map((sub) => (
                                    <tr key={sub._id} style={{ borderBottom: "1px solid rgba(255,255,255,0.04)" }}>
                                        <td style={{ padding: "var(--space-4)", fontWeight: "bold", color: "var(--color-text-primary)" }}>{sub.email}</td>
                                        <td style={{ padding: "var(--space-4)" }}>
                                            <span style={{ padding: "2px 8px", background: "rgba(16,185,129,0.1)", borderRadius: "var(--radius-sm)", color: "var(--color-success)", fontSize: "var(--font-size-xs)" }}>
                                                SUBSCRIBED
                                            </span>
                                        </td>
                                        <td style={{ padding: "var(--space-4)", color: "var(--color-text-tertiary)", fontSize: "var(--font-size-xs)" }}>
                                            {sub.createdAt ? new Date(sub.createdAt).toLocaleDateString() : "Recent"}
                                        </td>
                                    </tr>
                                ))
                            )}
                        </tbody>
                    </table>
                </div>
            </div>
        </div>
    );
};

export default NewsletterList;
