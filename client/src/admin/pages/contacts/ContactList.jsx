// APP/client/src/admin/pages/contacts/ContactList.jsx

import { useEffect, useState } from "react";
import { HiMail, HiRefresh } from "react-icons/hi";
import api from "../../../services/api.js";

const ContactList = () => {
    const [contacts, setContacts] = useState([]);
    const [loading, setLoading] = useState(true);

    const fetchContacts = () => {
        setLoading(true);
        api.get("/contact")
            .then((res) => setContacts(res.data?.data?.contacts || res.data?.data || []))
            .catch(() => {})
            .finally(() => setLoading(false));
    };

    useEffect(() => {
        fetchContacts();
    }, []);

    return (
        <div className="admin-page">
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "var(--space-6)", flexWrap: "wrap", gap: "var(--space-4)" }}>
                <div>
                    <h1 style={{ fontSize: "var(--font-size-2xl)", color: "var(--color-text-primary)", marginBottom: "var(--space-1)" }}>
                        Contact Messages
                    </h1>
                    <p style={{ fontSize: "var(--font-size-xs)", color: "var(--color-text-secondary)" }}>
                        View inquiry form submissions from potential clients.
                    </p>
                </div>
                <button type="button" className="btn btn--outline" onClick={fetchContacts}>
                    <HiRefresh /> Refresh
                </button>
            </div>

            <div style={{ background: "var(--color-bg-surface)", border: "1px solid var(--color-border)", borderRadius: "var(--radius-xl)", overflow: "hidden" }}>
                <div style={{ overflowX: "auto" }}>
                    <table style={{ width: "100%", borderCollapse: "collapse", textAlign: "left", fontSize: "var(--font-size-sm)" }}>
                        <thead>
                            <tr style={{ background: "rgba(255,255,255,0.02)", borderBottom: "1px solid var(--color-border)" }}>
                                <th style={{ padding: "var(--space-4)", color: "var(--color-text-tertiary)", fontWeight: "bold" }}>Sender Name</th>
                                <th style={{ padding: "var(--space-4)", color: "var(--color-text-tertiary)", fontWeight: "bold" }}>Email</th>
                                <th style={{ padding: "var(--space-4)", color: "var(--color-text-tertiary)", fontWeight: "bold" }}>Subject</th>
                                <th style={{ padding: "var(--space-4)", color: "var(--color-text-tertiary)", fontWeight: "bold" }}>Date</th>
                            </tr>
                        </thead>
                        <tbody>
                            {loading ? (
                                <tr>
                                    <td colSpan={4} style={{ padding: "var(--space-8)", textAlign: "center", color: "var(--color-text-secondary)" }}>
                                        Loading messages...
                                    </td>
                                </tr>
                            ) : contacts.length === 0 ? (
                                <tr>
                                    <td colSpan={4} style={{ padding: "var(--space-8)", textAlign: "center", color: "var(--color-text-secondary)" }}>
                                        <HiMail size={32} style={{ opacity: 0.3, marginBottom: "var(--space-2)" }} />
                                        <p>No messages received yet.</p>
                                    </td>
                                </tr>
                            ) : (
                                contacts.map((c) => (
                                    <tr key={c._id} style={{ borderBottom: "1px solid rgba(255,255,255,0.04)" }}>
                                        <td style={{ padding: "var(--space-4)", fontWeight: "bold", color: "var(--color-text-primary)" }}>{c.name}</td>
                                        <td style={{ padding: "var(--space-4)", color: "var(--color-accent)" }}>{c.email}</td>
                                        <td style={{ padding: "var(--space-4)", color: "var(--color-text-secondary)" }}>{c.subject}</td>
                                        <td style={{ padding: "var(--space-4)", color: "var(--color-text-tertiary)", fontSize: "var(--font-size-xs)" }}>
                                            {c.createdAt ? new Date(c.createdAt).toLocaleDateString() : "Recent"}
                                        </td>
                                    </tr>
                                ))
                            )}
                        </tbody>
                    </table>
                </div>
            </div>
        </div>
    );
};

export default ContactList;
